...
import { callGPT, callClaude, type Language } from "./llm";

// Iterio is EN-only — the language overlay is a no-op (kept so the ported
// per-mode functions compile unchanged).
function withLanguage(system: string, _language?: Language): string {
  return system;
}
...
export async function generateStudioFlowPrompt(
  crafterPrompt: string,
  language?: Language,
): Promise<string> {
  return callGPT({
    systemPrompt: withLanguage(STUDIO_FLOW_V2_SYSTEM, language),
...
export async function formatProductOnlyTemplate(
  ...
  const result = await callClaude({
    system: withLanguage(PRODUCT_ONLY_SYSTEM, language),
    messages: [
      {
        role: "user",
...
export async function formatDualRefTemplate(
  ...
  const result = await callClaude({
    system: withLanguage(DUAL_REF_SYSTEM, language),
    messages: [
      {
        role: "user",
...
export async function formatBrollTemplate(
  ...
  const result = await callClaude({
    system: withLanguage(BROLL_SYSTEM, language),
    messages: [
      {
        role: "user",
...
export async function formatArollStreetWithProductTemplate(
  ...
  const result = await callClaude({
    system: withLanguage(AROLL_STREET_WITH_PRODUCT_SYSTEM, language),
    messages: [
      {
        role: "user",
...
export async function formatArollStreetNoProductTemplate(
  ...
  const result = await callClaude({
    system: withLanguage(AROLL_STREET_NO_PRODUCT_SYSTEM, language),
    messages: [
      {
        role: "user",
...
export async function formatArollTalkingHeadTemplate(
  ...
  const result = await callClaude({
    system: withLanguage(AROLL_TALKING_HEAD_SYSTEM, language),
    messages: [
      {
        role: "user",
...
export async function formatArollPodcastWithRefsTemplate(
  ...
  const result = await callClaude({
    system: withLanguage(AROLL_PODCAST_WITH_REFS_SYSTEM, language),
    messages: [
      {
        role: "user",
...
export async function formatArollPodcastNoRefsTemplate(
  ...
  const result = await callClaude({
    system: withLanguage(AROLL_PODCAST_NO_REFS_SYSTEM, language),
    messages: [
      {
        role: "user",
...
export async function formatArollGreenScreenTemplate(
  ...
  const result = await callClaude({
    system: withLanguage(AROLL_GREEN_SCREEN_SYSTEM, language),
    messages: [
      {
        role: "user",
...
export async function formatNoRefTemplate(
  ...
  const result = await callClaude({
    system: withLanguage(NO_REF_SYSTEM, language),
    messages: [
      {
        role: "user",
        content: `Format this prompt into the template. Aspect ratio: ${aspectRatio}. Duration: ${duration} seconds.\n\n${cleanedPrompt}`,
...
export async function cleanVoiceDialogue(
  prompt: string,
  language?: Language,
): Promise<string> {
  const result = await callClaude({
    system: withLanguage(VOICE_CLEANUP_SYSTEM, language),
    messages: [
      {
        role: "user",
        content: prompt,
