...
import { buildMetaAdLibraryUrl, META_ADS_ACTOR_ID, type ScrapeMode } from "@/systems/competitor-research/meta-url";
...
export async function POST(req: Request) {
  ...
  const mode: ScrapeMode = body.mode === "keyword" ? "keyword" : "page_id";
  const country = (body.country || "ALL").trim();
  const requestedCount = Math.min(50, Math.max(1, body.requestedCount || 20));
  const url = buildMetaAdLibraryUrl({ mode, query: body.query.trim(), country });
